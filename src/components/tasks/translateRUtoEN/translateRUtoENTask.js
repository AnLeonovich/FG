import ruVocabulary from './russianVocabulary.json'
...
const TRANSLATE_TASK_HTML = require('./translateRUtoENTask.html');

let result; 

export class TranslateRUtoEN {
	constructor() {
    let rules = `Translate the word into russian`;
    let task = generateRandomObjProperty(ruVocabulary),
      answer = ruVocabulary[task];
  	this.showTask(rules, task, answer);
    delete ruVocabulary[task];
  }
